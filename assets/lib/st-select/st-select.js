...
$.fn.wisySelect = function () {
	...
	$('select.st-custom-select', context).each(function () {
		var $this = $(this)[0],
			...
			selectOptions = $($this.outerHTML).find('option'),
			selectedOption = $($this).find('option[value="'+$this.value+'"]')[0],
			customHTML;

		customHTML = '<div class="st-custom-select">';
			customHTML += '<div class="select">';
				customHTML += '<div class="selected"><span>';
				if ( typeof selectedOption.getAttribute('data-before') == 'string' )
				{
					customHTML += selectedOption.getAttribute('data-before')+' ';
				}
				customHTML += selectedOption.innerText+'</span><i class="feather icon-chevron-down"></i></div>';
				customHTML += '<div class="options-box">';
					customHTML += '<ul>';
						for (var i = 0; i <= (selectOptions.length - 1); i++) {
							...
							customHTML += ' data-value="'+value+'">';
							if ( typeof el.getAttribute('data-before') == 'string' )
							{
								customHTML += el.getAttribute('data-before');
							}
							customHTML += text+'</li>';
						}
					customHTML += '</ul>';
	...
	$(context).on('click', '.st-custom-select>.select>.options-box>ul>li', function (e) {
		...
		var option = {
			...
			txt: $Elem[0].innerHTML
		};

		$Elem.closestParents('.select').find('>.selected>span').html(option.txt);
